# ONNX-Accelerated-DialSort

ONNX-Accelerated-DialSort is an experimental ONNX Runtime variant of DialSort. It is intended for benchmarking and architecture experiments, not as a replacement for the native C++ DialSort implementation.
...
ONNX-Accelerated-DialSort models the ingestion phase as an ONNX graph. The graph receives a 1D `int64` tensor named `keys` and returns a 1D `int64` tensor named `histogram` with length `U`.

The C++ wrapper then projects that histogram back into the caller's `std::vector<int64_t>`.

Each `ONNXAcceleratedDialSort` instance keeps an ONNX Runtime session cache keyed by universe size. The
...
ONNXAcceleratedDialSort sorter;
...
`ONNXAcceleratedDialSort::sort` returns `true` only when the model is present, ONNX Runtime executes successfully, the model dimensions match the requested universe size, and the projected output is sorted. Recoverable failures return `false` instead of throwing to the caller.

## Safe API vs Benchmark API

`ONNXAcceleratedDialSort::sort()` validates that all input values are inside the valid universe range:

```text
[min_value, min_value + U - 1]
```

This is the recommended API for external callers.

`ONNXAcceleratedDialSort::sort_unchecked()` skips value-range validation and is intended only for trusted benchmark data generated internally by `bench_onnx_accelerated_dialsort.cpp`.

The benchmark uses `sort_unchecked()` because all generated distributions already produce values in `[0, U - 1]`, so repeating the full input validation would add an extra `O(n)` pass and make the measurement less comparable with the original web/Crow implementation.

External users should prefer `sort()`.
...
```bash
python onnx_accelerated_dialsort/scripts/test_dialsort_onnx.py \
  --model onnx_accelerated_dialsort/models/dialsort_U256.onnx \
  --array "3,1,3,5,1,3"
```
...
```bash
python onnx_accelerated_dialsort/scripts/test_dialsort_onnx.py \
  --model onnx_accelerated_dialsort/models/dialsort_U1024.onnx \
  --random-n 1000000
...
```bash
./build-onnx/bench_onnx_accelerated_dialsort
```

On multi-config generators such as Visual Studio:

```bash
./build-onnx/Release/bench_onnx_accelerated_dialsort.exe
```

The benchmark compares:

- `NativeCounting`, a native DialSort-style counting and projection baseline
- `ONNX-Accelerated-DialSort`, the ONNX Runtime path backed by the `ONNXAcceleratedDialSort` C++ wrapper
- `std::sort`

`NativeCounting` is a native CPU baseline, not the ONNX-Accelerated-DialSort parity target. Use the
`ONNX-Accelerated-DialSort` rows when evaluating the ONNX Runtime path.
...
Each row validates correctness with `std::is_sorted`. If a matching model is missing or the model cannot run, the `ONNX-Accelerated-DialSort` row is reported as skipped and the rest of the benchmark continues.

## Benchmark Methodology

The benchmark measures only the sorting function call.

Input generation is excluded from the measured time.

The benchmark performs warmup rounds before measurement so that ONNX Runtime session creation and model loading are amortized.

The same `ONNXAcceleratedDialSort` instance is reused across benchmark cases, allowing ONNX Runtime sessions to be cached per universe size.

The ONNX-Accelerated-DialSort benchmark path uses `sort_unchecked()` because benchmark inputs are generated internally and guaranteed to be in range.

Correctness is checked after each measured run.
...
`std::sort` is skipped for very large `N` to keep total benchmark runtime practical.
...
- The first call for a universe size pays the ONNX model load/session creation cost; reuse the same `ONNXAcceleratedDialSort` instance to benefit from the session cache.
- ONNX Runtime is a build-time and run-time dependency only when `DSORT_ENABLE_ONNX=ON`.
- Missing models, unsupported dimensions, invalid value ranges, and inference failures return `false` from `ONNXAcceleratedDialSort::sort()`.
