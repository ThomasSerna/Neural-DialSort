...
public:
    ...
    sort_result_dto sort(const sort_request_dto& request, const std::string& algorithm) const;
    std::vector<sort_result_dto> compare(const sort_request_dto& request) const;
...
private:
    ...
    sort_result_dto sortNeural(const std::vector<int64_t>& values, int64_t u) const;
};
