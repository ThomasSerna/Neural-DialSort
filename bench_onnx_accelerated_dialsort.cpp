#include "onnx_accelerated_dialsort.h"

#include <algorithm>
#include <chrono>
#include <cstdlib>
#include <cstdint>
#include <functional>
#include <iomanip>
#include <iostream>
#include <limits>
#include <random>
#include <string>
#include <utility>
#include <vector>

namespace {
constexpr int WARMUP_ROUNDS = 3;
constexpr int MEASURE_ROUNDS = 7;
constexpr uint64_t SEED = 20260321ULL;
constexpr int64_t MAX_U_COUNTING = 10'000'000;
constexpr std::size_t STD_SORT_SKIP_N = 1'000'000;
constexpr const char* ONNX_ACCELERATED_DIALSORT_NAME = "ONNX-Accelerated-DialSort";

std::string compiler_id() {
#if defined(__clang__)
    return std::string("Clang ") + __clang_version__;
#elif defined(__GNUC__)
    return std::string("g++ ") + __VERSION__;
#elif defined(_MSC_VER)
    return std::string("MSVC ") + std::to_string(_MSC_VER);
#else
    return "unknown compiler";
#endif
}


int64_t now_ns() {
    using namespace std::chrono;
    return duration_cast<nanoseconds>(high_resolution_clock::now().time_since_epoch()).count();
}

bool dialsort_counting(std::vector<int64_t>& values, int64_t universe_size) {
    if (values.size() <= 1) {
        return true;
    }

    if (universe_size <= 0 || universe_size > MAX_U_COUNTING) {
        return false;
    }

    std::vector<int64_t> histogram(static_cast<std::size_t>(universe_size), 0);
    for (int64_t value : values) {
        if (value < 0 || value >= universe_size) {
            return false;
        }
        ++histogram[static_cast<std::size_t>(value)];
    }

    std::size_t out = 0;
    for (int64_t value = 0; value < universe_size; ++value) {
        const int64_t count = histogram[static_cast<std::size_t>(value)];
        for (int64_t i = 0; i < count; ++i) {
            values[out++] = value;
        }
    }

    return out == values.size();
}

std::vector<int64_t> gen_uniform(std::size_t n, int64_t universe_size, uint64_t seed) {
    std::mt19937_64 rng(seed);
    std::uniform_int_distribution<int64_t> dist(0, universe_size - 1);
    std::vector<int64_t> values(n);
    for (int64_t& value : values) {
        value = dist(rng);
    }
    return values;
}

std::vector<int64_t> gen_skewed(std::size_t n, int64_t universe_size, uint64_t seed) {
    std::mt19937_64 rng(seed);
    const int64_t hot_limit = std::max<int64_t>(1, universe_size / 20);
    std::uniform_int_distribution<int64_t> hot(0, hot_limit - 1);
    std::uniform_int_distribution<int64_t> cold(0, universe_size - 1);
    std::bernoulli_distribution pick_hot(0.80);

    std::vector<int64_t> values(n);
    for (int64_t& value : values) {
        value = pick_hot(rng) ? hot(rng) : cold(rng);
    }
    return values;
}

std::vector<int64_t> gen_sorted(std::size_t n, int64_t universe_size, uint64_t seed) {
    auto values = gen_uniform(n, universe_size, seed);
    if (n >= STD_SORT_SKIP_N) {
        dialsort_counting(values, universe_size);
    } else {
        std::sort(values.begin(), values.end());
    }
    return values;
}

std::vector<int64_t> gen_reverse(std::size_t n, int64_t universe_size, uint64_t seed) {
    auto values = gen_sorted(n, universe_size, seed);
    std::reverse(values.begin(), values.end());
    return values;
}

using SortFn = std::function<bool(std::vector<int64_t>&)>;

struct Row {
    std::string algorithm;
    std::string distribution;
    std::size_t n = 0;
    int64_t universe_size = 0;
    int64_t best_ns = 0;
    double ms = 0.0;
    double mkeys_s = 0.0;
    double speedup_vs_std_sort = 0.0;
    bool speedup_available = false;
    bool correct = false;
    bool skipped = false;
    std::string skip_reason;
};

Row run_one(
    const std::string& algorithm,
    const std::string& distribution,
    const std::vector<int64_t>& base,
    int64_t universe_size,
    const SortFn& sort_fn,
    int64_t std_sort_ns
) {
    Row row;
    row.algorithm = algorithm;
    row.distribution = distribution;
    row.n = base.size();
    row.universe_size = universe_size;

    for (int i = 0; i < WARMUP_ROUNDS; ++i) {
        auto tmp = base;
        if (!sort_fn(tmp) || !std::is_sorted(tmp.begin(), tmp.end())) {
            row.skipped = true;
            return row;
        }
    }

    int64_t best = std::numeric_limits<int64_t>::max();
    bool correct = true;

    for (int i = 0; i < MEASURE_ROUNDS; ++i) {
        auto tmp = base;
        const int64_t start = now_ns();
        const bool ran = sort_fn(tmp);
        const int64_t elapsed = now_ns() - start;

        if (!ran) {
            row.skipped = true;
            return row;
        }

        correct = correct && std::is_sorted(tmp.begin(), tmp.end());
        if (elapsed > 0 && elapsed < best) {
            best = elapsed;
        }
    }

    if (best == std::numeric_limits<int64_t>::max()) {
        row.skipped = true;
        return row;
    }

    row.best_ns = best;
    row.ms = static_cast<double>(best) / 1e6;
    row.mkeys_s = (static_cast<double>(base.size()) / (static_cast<double>(best) / 1e9)) / 1e6;
    if (std_sort_ns > 0) {
        row.speedup_vs_std_sort = static_cast<double>(std_sort_ns) / static_cast<double>(best);
        row.speedup_available = true;
    }
    row.correct = correct;
    return row;
}

Row skipped_row(
    const std::string& algorithm,
    const std::string& distribution,
    std::size_t n,
    int64_t universe_size,
    std::string reason
) {
    Row row;
    row.algorithm = algorithm;
    row.distribution = distribution;
    row.n = n;
    row.universe_size = universe_size;
    row.skipped = true;
    row.skip_reason = std::move(reason);
    return row;
}

static constexpr int COL_W = 109;

void print_separator() {
    std::cout << std::string(COL_W, '-') << "\n";
}

void print_header() {
    std::cout << std::left
              << std::setw(28) << "Algorithm"
              << std::setw(12) << "Distribution"
              << std::setw(12) << "N"
              << std::setw(8)  << "U"
              << std::setw(12) << "ms (best)"
              << std::setw(14) << "M keys/s"
              << std::setw(12) << "Speedup"
              << "OK\n";
    print_separator();
}

void print_row(const Row& row) {
    if (row.skipped) {
        std::cout << std::left
                  << std::setw(28) << row.algorithm
                  << std::setw(12) << row.distribution
                  << std::setw(12) << row.n
                  << std::setw(8)  << row.universe_size;

        if (!row.skip_reason.empty()) {
            std::cout << "[SKIPPED - " << row.skip_reason << "]";
        } else if (row.algorithm == ONNX_ACCELERATED_DIALSORT_NAME) {
            std::cout << "[SKIPPED - model/runtime unavailable]";
        } else if (row.algorithm == "NativeCounting") {
            std::cout << "[SKIPPED - U=" << row.universe_size
                      << " > MAX_U_COUNTING=" << MAX_U_COUNTING << "]";
        } else {
            std::cout << "[SKIPPED]";
        }

        std::cout << "\n";
        return;
    }

    std::cout << std::left
              << std::setw(28) << row.algorithm
              << std::setw(12) << row.distribution
              << std::setw(12) << row.n
              << std::setw(8)  << row.universe_size
              << std::fixed << std::setprecision(3)
              << std::setw(12) << row.ms
              << std::setw(14) << row.mkeys_s;

    if (row.speedup_available) {
        std::cout << std::setw(12) << row.speedup_vs_std_sort;
    } else {
        std::cout << std::setw(12) << "n/a";
    }

    std::cout << (row.correct ? "OK" : "*** FAIL ***") << "\n";
}

struct Dist {
    std::string name;
    std::function<std::vector<int64_t>(std::size_t, int64_t, uint64_t)> generate;
};

struct Args {
    std::string model_dir = "onnx_accelerated_dialsort/models";
};

Args parse_args(int argc, char** argv) {
    Args args;
    for (int i = 1; i < argc; ++i) {
        const std::string current = argv[i];
        if (current == "--model-dir" && i + 1 < argc) {
            args.model_dir = argv[++i];
        } else if (current == "--help" || current == "-h") {
            std::cout << "Usage: bench_onnx_accelerated_dialsort [--model-dir PATH]\n";
            std::exit(0);
        }
    }
    return args;
}
}

int main(int argc, char** argv) {
    const Args args = parse_args(argc, argv);

    ONNXAcceleratedDialSort onnx_dialsort({
        args.model_dir,
        0,
        8,
    });

    const std::vector<std::size_t> ns = {10'000, 100'000, 1'000'000, 10'000'000};
    const std::vector<int64_t> universe_sizes = {256, 1024, 65536};
    const std::vector<Dist> distributions = {
        {"uniform", gen_uniform},
        {"skewed", gen_skewed},
        {"sorted", gen_sorted},
        {"reverse", gen_reverse},
    };

    std::cout
            << "================================================================\n"
            << "ONNX-Accelerated DialSort — Optional ONNX Benchmark  (Tier 1: CPU)\n"
            << "Paper: DialSort: Non-Comparative Integer Sorting\n"
            << "       via the Self-Indexing Principle\n"
            << "================================================================\n"
            << "Compiler     : " << compiler_id() << "\n"
#ifdef NDEBUG
            << "Build mode   : Release / NDEBUG\n"
#else
            << "Build mode   : Debug / no NDEBUG\n"
#endif
            << "Warmup       : " << WARMUP_ROUNDS << " discarded runs\n"
            << "Measurement  : best-of-" << MEASURE_ROUNDS << " runs\n"
            << "Seed         : " << SEED << "\n"
            << "Clock        : std::chrono::high_resolution_clock\n"
            << "Correctness  : std::is_sorted() verified after every sort\n"
            << "Model dir    : " << args.model_dir << "\n"
            << "Max U (count): " << MAX_U_COUNTING
            << "  (~" << (MAX_U_COUNTING * sizeof(int64_t)) / (1024 * 1024)
            << " MB histogram; NativeCounting rows skipped if U exceeds this)\n"
            << "Note         : std::sort is skipped for N >= " << STD_SORT_SKIP_N
            << " to keep ONNX benchmark runtime practical\n"
            << "================================================================\n\n";

    std::cout << "TABLE I - Bounded-universe sorting  "
                 "(ONNX-Accelerated DialSort vs baselines)\n";
    std::cout << "Each group: NativeCounting | ONNX-Accelerated-DialSort | std::sort\n\n";

    print_header();

    for (std::size_t n : ns) {
        for (int64_t universe_size : universe_sizes) {
            for (const auto& dist : distributions) {
                const uint64_t seed = SEED ^
                                      (static_cast<uint64_t>(n) * 1000003ULL) ^
                                      (static_cast<uint64_t>(universe_size) * 7919ULL);
                const auto base = dist.generate(n, universe_size, seed);

                const bool run_std_sort = n < STD_SORT_SKIP_N;
                Row std_row;
                int64_t std_sort_ns = 0;
                if (run_std_sort) {
                    const SortFn std_sort = [](std::vector<int64_t>& values) {
                        std::sort(values.begin(), values.end());
                        return true;
                    };

                    std_row = run_one(
                        "std::sort",
                        dist.name,
                        base,
                        universe_size,
                        std_sort,
                        1
                    );
                    std_row.speedup_vs_std_sort = 1.0;
                    std_row.speedup_available = true;
                    std_sort_ns = std_row.best_ns;
                } else {
                    std_row = skipped_row(
                        "std::sort",
                        dist.name,
                        base.size(),
                        universe_size,
                        "large N"
                    );
                }

                const SortFn native_sort = [universe_size](std::vector<int64_t>& values) {
                    return dialsort_counting(values, universe_size);
                };

                const SortFn onnx_sort = [&onnx_dialsort, universe_size](std::vector<int64_t>& values) {
                    return onnx_dialsort.sort_unchecked(values, universe_size);
                };

                const Row native_row = run_one(
                    "NativeCounting",
                    dist.name,
                    base,
                    universe_size,
                    native_sort,
                    std_sort_ns
                );

                const Row onnx_row = run_one(
                    ONNX_ACCELERATED_DIALSORT_NAME,
                    dist.name,
                    base,
                    universe_size,
                    onnx_sort,
                    std_sort_ns
                );

                print_row(native_row);
                print_row(onnx_row);
                print_row(std_row);
                std::cout << "\n";
            }
        }
    }

    return 0;
}
